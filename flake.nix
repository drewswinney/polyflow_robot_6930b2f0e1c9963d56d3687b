{
  ...
  let
    ...
    rosOverlays =
      if builtins.isList rosBase then rosBase
      else if rosBase ? overlays then
        (if builtins.isList rosBase.overlays then rosBase.overlays else [ rosBase.overlays ])
      else [ rosBase ];

    # pkgs with ROS overlays + Python pin
    pkgs = import nixpkgs {
      inherit system;
      overlays = rosOverlays ++ [ pinPython312 ];
    };
...
EOF
  ...
  in
  {
    ...
    packages.${system} = {
      ...
      # rosWorkspaceEnv = rosWorkspace.env;
    };
